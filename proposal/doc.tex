...
We will first reproduce the results of the Word Translation Without Parallel Data
paper on the Japanese-English language pair, using fastText Wikipedia embeddings as
the pre-trained word embedding spaces to align.
...
we will instead use a purely supervised approach to generate the noisy pairs,
and if that does not suffice, an actual labeled dictionary.

While the code for this paper is released, the implementation is in PyTorch, so
TensorFlow will be used instead for novelty.
